...
export const LessonProgressController = {
  async setVideoProgress(req: Request, res: Response) {
    try {
      ...
      const saved = await LessonProgressService.upsertVideoProgress(
        studentId, lessonId, watchedMs || 0, durationMs || 0, isCompleted
      );
  ...
  async setDocumentRead(req: Request, res: Response) {
    try {
      const lessonId = decodeURIComponent(req.params.lessonId);
