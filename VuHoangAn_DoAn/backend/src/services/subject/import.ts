...
async function parseExcel(filePath: string): Promise<SubjectRow []> {
  const buf = await fs.readFile(filePath);
  const wb = XLSX.read(buf, { type: 'buffer' });
  const ws = wb.Sheets[wb.SheetNames[0]];
  const rows = XLSX.utils.sheet_to_json<SubjectRow>(ws, { defval: '' });
  return rows;
}

function normalizeAndValidate(rows: SubjectRow[]) {
  const valid: ValidSubject[] = [];
...
async function upsertSubjects(data: ValidSubject[]) {
