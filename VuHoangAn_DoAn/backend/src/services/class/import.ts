...
async function parseExcel(filePath: string): Promise<ClassRow[]> {
  const buf = await fs.readFile(filePath);
  const wb = XLSX.read(buf, { type: 'buffer' });
  const ws = wb.Sheets[wb.SheetNames[0]];
  const rows = XLSX.utils.sheet_to_json<ClassRow>(ws, { defval: '' });
  return rows;
}

function normalizeAndValidate(rows: ClassRow[]) {
  const valid: ValidClass[] = [];
...
async function upsertClasses(data: ValidClass[]) {
