...
export const CourseService ={
    ...
    async create(data: { courseId: string; courseName: string; description: string; }) {
        const existingCourse = await Course.findOne({ courseId: data.courseId });
        if (existingCourse) throw new Error(`Course with ID ${data.courseId} already exists`);
    ...
    async addClass(courseId: string, classId: string){
        ...
        course.classes.push(cls._id as any);
    ...
    async removeClass(courseId: string, classId: string){
