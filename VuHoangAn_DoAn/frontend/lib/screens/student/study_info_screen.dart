...
class StudyInfoScreen extends StatefulWidget {
  ...
  const StudyInfoScreen({super.key, required this.className});
...
String _formatTimeRange(BuildContext ctx, Schedule s) {
  ...
  final List<DateTime> days = List.generate(7, (i) => from.add(Duration(days: i)));
  final items = <Widget>[];
  ...
  Container(
    ...
    decoration: BoxDecoration(
      ...
      boxShadow: [BoxShadow(color: Colors.grey.withOpacity(0.1), blurRadius: 4, spreadRadius: 1, offset: const Offset(0, 1))],
    ),
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(dayLabel, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
        const SizedBox(height: 4),
        Text(s.subjectName, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w500)),
        const SizedBox(height: 2),
        Text('$timeStr  •  ${s.room ?? "-"}', style: const TextStyle(fontSize: 14, color: Colors.black54)),
        if (s.lecturer != null && s.lecturer!.isNotEmpty)
          Align(
            alignment: Alignment.centerRight,
            child: Text(s.lecturer!, style: const TextStyle(fontSize: 12, color: Colors.black54)),
          ),
      ],
    ),
  ),
  ...
  title: Text('Thời khóa biểu • ${widget.className}',
      style: const TextStyle(color: Colors.black, fontWeight: FontWeight.w600)),
  centerTitle: false,
  actions: [
    IconButton(onPressed: _prevWeek, icon: const Icon(Icons.chevron_left, color: Colors.black87)),
    Center(
      child: Text('${from.day}/${from.month} - ${to.day}/${to.month}',
        style: const TextStyle(color: Colors.black87, fontWeight: FontWeight.w500)),
    ),
    IconButton(onPressed: _nextWeek, icon: const Icon(Icons.chevron_right, color: Colors.black87)),
  ],
),
    body: _loading
